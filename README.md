...
`<!DOCTYPE>`은 웹 브라우저에 해당 문서가 어떤 HTML 버전을 기바능로 작성되었는지 알려주는 역할을 하는 선언문이다. 주로 문서 최상단에 위치하며, 브라우저가 HTML 문소를 해석하고 렌더링 하는 방식을 결정한다.

과거에는 HTML의 다양한 버전이 존재했기 때문에 브라우저가 문서를 올바른 방식으로 해석하기 위해 적절한 방식으로 직접 지정해야했다. HTML5에 접어들면서 선언 방식이 단순화되어 간단하게 선언할 수 있다. 

이 선언문은 HTM5를 사용하고 있음을 명시한다.

Doctype을 선언하지 않으면 브라우저는 문서를 쿼크모드로 렌더링 할 수 있다. 쿼크 모드는 오래된 웹사이트와의 호환성을 유지하기 위해 표준과 다른 방식으로 동작하기 때문에 예상치 못한 동작을 발생시킬 수 있다.
...
<details>
<summary><strong>프로토타입 상속의 동작 방식이란?</strong><summary>

프로토타입은 자바스크립트에서 객체 간의 상속을 구현하는 메커니즘이다.

자바스크립트의 모든 객체는 기본적으로 `[[Prototype]]`이라는 숨김 프로퍼티를 가지고 있으며, 이 프로퍼티는 다른 객체를 참조하거나 `null`값을 가진다. 프로토타입 연결은 `Object.create()`나 함수 생성자의 prototype 프로퍼티를 통해 이루어진다.

프로토타입 상속이 동작하는 방식은 프로토타입 체인을 기반으로 한다.

객체에서 어떤 프로퍼티를 접근하려고 할 때, 자바스크립트 엔진은 해당 객체에서 프로퍼티를 찾는다. 만약 찾을 수 없다면 객체의 `[[Prototype]]`이 가리키는 프로토타입 객체에서 프로퍼티를 탐색한다.

만약 프로토타입 객체에서도 해당 프로퍼티를 찾지 못하면, 그 다음에는 프로토타입의 프로토타입을 탐색한다. 탐색 과정을 계속 반복하면서 원하는 프로퍼티를 찾거나, 프로토타입이 `null`이 되는 단계에 도달할 때까지 프로토타입 체인을 타고 올라가는 방식으로 탐색한다.
</details>

<details>
<summary><strong>각 이미지 포맷 별 특징</strong><summary>

웹에서 자주 활용되는 이미지 포맷은 JPEG, PNG, WebP, SVG가 있다.

- JPEG: 손실 압축 방식을 사용하는 이미지 포맷으로, 파일 크기를 효율적으로 줄일 수 있다는 장점이 있다. 이미지를 저장할 때마다 데이터가 손실되어 화질이 점점 나빠지는 현상이 발생한다는 단점이 있다. 복잡한 색상과 그라데이션이 있는 이미지에 적합한 이미지 포맷이다.

- PNG: 무손실 압축 방식을 사용하는 이미지 포맷으로, 투명도를 지원하며 JPEG보다 파일 크기가 큰 편이다. 선명한 텍스트나 투명 색상이 있는 이미지에 적합하다.

- WebP: 손실/무손실 압축을 모두 지원하며 투명도도 지원한다. JPEG나 PNG보다 더 나은 압축률을 제공하면서도 화질을 유지할 수 있어서 웹 최적화에 적합하다.

- SVG: XML 형식으로 이루어져 있다. 크기를 조절해도 이미지 품질 저하가 없고 파일 크기가 작다는 장점이 있다. 로고나 아이콘, 단순한 일러스트레이션에 사용된다.
...
<details>
<summary><strong>CORS 설정 없이 SOP를 우회하여 외부 서버와 통신할 수 있는 방법은?</strong></summary>
프록시 서버를 이용한다면 CORS 설정 없이도 SOP를 우회할 수 있다. 프록시 서버란 브라우저 대신 외부 서버에 요청을 보내고 응답을 받는 역할을 대리 수행하는 서버이다.
브라우저 측에서 직접 외부 서버에 요청을 보내지 않고, 클라이언트와 동일한 origin의 프록시 서버를 통해 요청을 보내면 SOP의 제한을 피할 수 있다.

클라이언트 측 도메인과 서버 측 도메인이 다를 경우, CORS 설정을 별도로 하지 않앗다면, 도메인이 다르므로 브라우저 단에서 SOP에 의해 통신이 차단된다.

하지만 브라우저가 아닌 클라이언트 서버를 통해 요청을 보낸다면 서버와 서버 간의 통신에는 SOP가 적용되지 않기 때문에 응답을 받을 수 있게 된다. 
이렇게 하면 클라이언트 측 origin과 서버 측 origin이 일치하기 때문에 정상적으로 응답을 수신할 수 있게 된다.
</details>
...
<details>
<summary><strong>전역 상태관리 라이브러리를 사용하는 이유는?</strong></summary>

1. 컴포넌트 간 상태 공유가 용이해짐: 여러 컴포넌트에서 공통적으로 사용되는 상태를 중앙화하여 관리하고, 여러 곳에서 쉽게 접근할 수 있다.

2. 관심사 분리가 용이해짐: 상태 관리 로직을 컴포넌트에서 분리하여 별도로 관리함으로써, 컴포넌트는 UI 로직에만 집중할 수 있게 됨

3. 성능 최적화: 불필요한 리렌더링을 방지하는 메커니즘을 제공하기 때문
</details>
...
<details>
<summary><strong>대칭키 및 비대칭키 암호화 방식은?</strong></summary>

암복호화에 사용하는 키가 동일한 경우 대칭키 암호화라고 하며, 암호화에 사용하는 키가 서로 다른 경우를 비대칭키 암호화 또는 공개키 암호화라고 한다.

대칭키 암호화는 비대칭키 암호화에 비해서 속도가 빠르다고 알려져 있지만, 대칭키를 교환하는 과정에서 탈취 위험성이 존재할 수 있다.
